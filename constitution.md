...
- **2.2 (参数化测试):** 单元测试必须优先采用pytest的参数化测试（`@pytest.mark.parametrize`）的风格。
...
**核心：** 代码的首要目的是让人类易于理解。
- **3.1 (错误处理):** **不可协商**：所有错误都必须被显式处理。错误传递时必须使用`raise ValueError("message") from err`进行包装。
